...
- overflow: `overflow-hidden` alone, clipping only. A scrollable pane's home is a shipped pane
  (`ScrollArea` on web, the `ScrollView` host on native), with consumer `ui/` the fallback.
...
pane's home is `ScrollArea` on web or the `ScrollView` host on native, with consumer `ui/` the
fallback), margins including
