...
import type { CodegenWranglerPayload } from "@fcalell/plugin-cloudflare";
...
>(
	...
	return bus.emit(cloudflare.events.Wrangler, emptyWrangler());
}
...
describe("binding collection across plugins", () => {
	...
	it("auth plugin contributes 2 rate limiters + 2 secrets via cloudflare.events.Wrangler", async () => {
	...
	it("event-based aggregation combines bindings and secrets from multiple plugins", async () => {
		...
		const wrangler = await bus.emit(
			cloudflare.events.Wrangler,
			emptyWrangler(),
		);
