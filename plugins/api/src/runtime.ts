...
export function createWorker<T extends readonly PluginConfig[]>(
	config: StackConfig<T>,
): AppBuilder<BaseContext> {
	const apiPlugin = config.plugins.find((p) => p.__plugin === "api");
	const apiOptions = (apiPlugin?.options ?? { prefix: "/rpc" }) as ApiOptions;

	return createAppBuilder<BaseContext>([], apiOptions, config);
}
	...
	return {
		use(
			...
			if (typeof pluginOrFn === "function") {
				...
				return createAppBuilder<any>(newEntries, apiOptions, config);
			}
