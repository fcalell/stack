...
```tsx
import { InputOTP, REGEXP_ONLY_DIGITS } from "@fcalell/plugin-solid-ui/components/input-otp";
```
...
Works with `Form.InputOTP` from `@fcalell/plugin-solid-ui/components/form` for TanStack Form integration.
