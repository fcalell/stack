...
// width up to three fifths and wraps inside it past that, at any character,
// so a key or an address never runs under the label; the label takes the
// rest.
...
export function DefinitionRow(props: DefinitionRowProps) {
	...
	const body = () => (
		<>
			<span class="flex w-full items-center gap-stack">
				<span class={cn(text({ role: "body" }), "min-w-0 flex-1")}>
					{props.label}
				</span>
				<Show when={props.value !== undefined}>
					<span class="flex max-w-3/5 shrink-0 items-center justify-end gap-row text-right wrap-anywhere">
